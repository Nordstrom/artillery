...
function run(targetScript, options) {
  ...
  async.waterfall(
    ...
    function done(err, script) {
      ...
      let runner = createRunner(script, payload, {
...
function readPayload(script, callback) {
  async.map(
    ...
    function readPayloadFile(payloadSpec, next) {
      let lines = fs.readFileSync(payloadSpec.path, 'utf-8').toString().split('\n')

      payloadSpec.data = lines
        .filter(line => line.length > 0)
        .map(line => line.split(','))

      return next(null, payloadSpec)
    },
    function done(err, results) {
      ...
      if (script.config.payload) {
        console.log('>>> DELETE_CONFIG_PAYLOAD')
        delete script.config.payload;
      }
